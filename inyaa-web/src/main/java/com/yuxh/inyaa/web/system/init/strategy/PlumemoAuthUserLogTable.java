...
@Service
@Slf4j
public class PlumemoAuthUserLogTable implements TableInfoService {

    @Override
    public void builderTable(final java.sql.Statement stat) {
        try {
            stat.execute(createHelloBlogAuthUserLog());
            log.info("初始化plumemo_auth_user_log完成");
        } catch (final SQLException e) {
            log.info("初始化plumemo_auth_user_log失败", e);
        }
    }

    @PostConstruct
    public void beforePropertiesSet() throws Exception {
        TableInitFactory.register("plumemo_auth_user_log", this);
    }

    private static String createHelloBlogAuthUserLog() {
        return "CREATE TABLE `plumemo_auth_user_log` (\n" +
