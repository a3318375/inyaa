...
@Service
@Slf4j
public class PlumemoConfigTable implements TableInfoService {
    ...
    public void builderTable(final java.sql.Statement stat) {
        try {
            ...
            log.info("初始化plumemo_config完成");
        } catch (final SQLException e) {
            log.error("初始化plumemo_config失败", e);
        }
    }

    @PostConstruct
    public void beforePropertiesSet() throws Exception {
        TableInitFactory.register("plumemo_config", this);
    }

    private static String createHelloBlogConfig() {
        return "CREATE TABLE `plumemo_config`  (\n" +
    ...
    private static String initData() {
        return "INSERT INTO `plumemo_config` VALUES " +
                ...
                "(5, 'metas', '1.1.1', 0)," +
                "(6, 'copyright', 'Copyright @ Plumemo', 0)," +
                "(7, 'icp', 'xxxx', 0)," +
