...
@Service
@Slf4j
public class PlumemoPostsCommentsTable implements TableInfoService, InitializingBean {

    @Override
    public void builderTable(final java.sql.Statement stat) {
        try {
            stat.execute(createHelloBlogPostsComments());
            log.info("初始化plumemo_posts_comments完成");
        } catch (final SQLException e) {
            log.error("初始化plumemo_posts_comments失败", e);
        }
    }

    @Override
    public void afterPropertiesSet() throws Exception {
        TableInitFactory.register("plumemo_posts_comments", this);
    }

    private static String createHelloBlogPostsComments() {
        return "CREATE TABLE `plumemo_posts_comments`  (\n" +
