...
@Service
@Slf4j
public class PlumemoAuthTokenTable implements TableInfoService {

    @Override
    public void builderTable(final java.sql.Statement stat) {
        try {
            stat.execute(createHelloBlogAuthToken());
            log.info("初始化plumemo_auth_token完成");
        } catch (final SQLException e) {
            log.error("初始化plumemo_auth_token失败", e);
        }
    }

    @PostConstruct
    public void beforePropertiesSet() {
        TableInitFactory.register("plumemo_auth_token", this);
    }

    private static String createHelloBlogAuthToken() {
        return "CREATE TABLE `plumemo_auth_token`  (\n" +
