...
@Service
@Slf4j
public class PlumemoFriendshipLinkTable implements TableInfoService, InitializingBean {

    @Override
    public void builderTable(final java.sql.Statement stat) {
        try {
            stat.execute(createHelloBlogFriendshipLink());
            log.info("初始化plumemo_friendship_link完成");
        } catch (final SQLException e) {
            log.error("初始化plumemo_friendship_link失败", e);
        }
    }

    @Override
    public void afterPropertiesSet() throws Exception {
        TableInitFactory.register("plumemo_friendship_link", this);
    }

    private static String createHelloBlogFriendshipLink() {
        return "CREATE TABLE `plumemo_friendship_link`  (\n" +
