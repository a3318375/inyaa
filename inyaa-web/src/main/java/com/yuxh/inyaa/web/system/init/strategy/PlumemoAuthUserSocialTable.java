...
@Service
@Slf4j
public class PlumemoAuthUserSocialTable implements TableInfoService {

    @Override
    public void builderTable(final java.sql.Statement stat) {
        try {
            stat.execute(createHelloBlogAuthUserSocial());
            log.info("初始化plumemo_auth_user_social完成");
        } catch (final SQLException e) {
            log.info("初始化plumemo_auth_user_social失败", e);
        }
    }

    @PostConstruct
    public void beforePropertiesSet(){
        TableInitFactory.register("plumemo_auth_user_social", this);
    }

    private static String createHelloBlogAuthUserSocial() {
        return "CREATE TABLE `plumemo_auth_user_social` (\n" +
