...
@Service
@Slf4j
public class PlumemoCategoryTagsTable implements TableInfoService, InitializingBean {

    @Override
    public void builderTable(final java.sql.Statement stat) {
        try {
            stat.execute(createHelloBlogCategoryTags());
            log.info("初始化plumemo_category_tags完成");
        } catch (final SQLException e) {
            log.error("初始化plumemo_category_tags失败", e);
        }
    }

    @Override
    public void afterPropertiesSet() throws Exception {
        TableInitFactory.register("plumemo_category_tags", this);
    }

    private static String createHelloBlogCategoryTags() {
        return "CREATE TABLE `plumemo_category_tags`  (\n" +
