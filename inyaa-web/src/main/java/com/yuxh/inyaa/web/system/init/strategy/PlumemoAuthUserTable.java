...
@Service
@Slf4j
public class PlumemoAuthUserTable implements TableInfoService, InitializingBean {

    private static String createHelloBlogAuthUser() {
        return "CREATE TABLE `plumemo_auth_user` (\n" +
    ...
    @Override
    public void builderTable(java.sql.Statement stat) {
        try {
            stat.execute(createHelloBlogAuthUser());
            log.info("初始化plumemo_auth_user完成");
        } catch (SQLException e) {
            log.error("初始化plumemo_auth_user失败", e);
        }
    }

    @Override
    public void afterPropertiesSet() throws Exception {
        TableInitFactory.register("plumemo_auth_user", this);
    }
}
