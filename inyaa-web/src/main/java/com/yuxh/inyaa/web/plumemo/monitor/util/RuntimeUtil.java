package com.yuxh.inyaa.web.plumemo.monitor.util;
...
//            obj.put("javaLibraryPath",props.getProperty("java.library.path"));// 加载库时搜索的路径列表
    ...
    public static Result<JSONObject> getUserCount() {
        JSONObject jso = new JSONObject();
        jso.put("commentsTotal", 0);
        jso.put("favorsTotal", 0);
        jso.put("viewsTotal", 0);
        jso.put("articleTotal", 0);
        return Result.createWithModel(jso);
    }

    public static Result<JSONObject> getUserChat() {
        Calendar calendar = Calendar.getInstance();
        List<JSONObject> list = new ArrayList<>();
        for (int i = 0; i < 12; i++) {
            calendar.set(Calendar.HOUR_OF_DAY, calendar.get(Calendar.HOUR_OF_DAY) - 2);
            JSONObject jso = new JSONObject();
            jso.put("createTime", calendar.getTime());
            jso.put("featured", 0);
            jso.put("viewsTotal", 0);
            list.add(jso);
        }
