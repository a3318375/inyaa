...
@Service
@Slf4j
public class PlumemoPostsTable implements TableInfoService, InitializingBean {

    @Override
    public void builderTable(final java.sql.Statement stat) {
        try {
            stat.execute(createHelloBlogPosts());
            log.info("初始化plumemo_posts完成");
        } catch (final SQLException e) {
            log.error("初始化plumemo_posts失败", e);
        }
    }

    @Override
    public void afterPropertiesSet() throws Exception {
        TableInitFactory.register("plumemo_posts", this);
    }

    private static String createHelloBlogPosts() {
        return "CREATE TABLE `plumemo_posts`  (\n" +
