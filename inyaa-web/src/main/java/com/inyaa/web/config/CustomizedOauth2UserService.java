...
public class CustomizedOauth2UserService extends DefaultOAuth2UserService {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) {
        ...
        if (QQ.equals(registrationId)) {
            ...
            // openId请求
            RequestEntity<?> openIdRequest = RequestEntity.get(UriComponentsBuilder.fromUriString(QQ_OPEN_ID_URL).queryParam("access_token", tokenValue)
                    .build()
                    .toUri()).build();

            // openId响应
            ResponseEntity<String> openIdResponse = restTemplate.exchange(openIdRequest, new ParameterizedTypeReference<String>() {
            });
            ...
            // userInfo请求
            RequestEntity<?> userInfoRequest = RequestEntity.get(UriComponentsBuilder.fromUriString(clientRegistration.getProviderDetails().getUserInfoEndpoint().getUri())
            ...
            // userInfo响应
            ResponseEntity<String> userInfoResponse = restTemplate.exchange(userInfoRequest, new ParameterizedTypeReference<String>() {
            });
            ...
            Map<String, Object> userAttributes = extractQqUserInfo(Objects.requireNonNull(userInfoResponse.getBody()));
